...
void smoke_batch_run_post_boot_tests(Serial_t *com1)
{
    ...
    if (KERNEL_SMOKE_TEST_ENABLE_DEBUG_EXCEPTION)
        smoke_test_run_debug_exception();
    if (KERNEL_SMOKE_TEST_ENABLE_BREAKPOINT)
        smoke_test_run_breakpoint_exception();
    if (KERNEL_SMOKE_TEST_ENABLE_INVALID_OPCODE)
        smoke_test_run_invalid_opcode_exception();
    if (KERNEL_SMOKE_TEST_ENABLE_GENERAL_PROTECTION)
        smoke_test_run_general_protection_exception();
